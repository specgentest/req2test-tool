...
open class ListMockTestCasePartialUpdateEndpointFilter (data: MutableMap<String, Any?>): BaseListMockTCEndpointFilter(data){

    override fun filterKeyInput(): String { return "ListPartialUpdateEndpointMockScript" }
    override fun filterName(): String {

        return "ListMockTestCasePartialUpdateEndpointFilter"
    }
    ...
    private fun createTestCaseValid200(ruleDataTest: ListDataTest, script: Script, entity: Entity): TestCase {
        ...
        steps.add("Call method ${testArchitecture.controllerLayerMethod} mapped by PATCH $endpointPartialUpdateWithId in ${testArchitecture.controllerLayerClass} class and return an updated instance of ${entity.name}: $output")
        ...
        steps.add("Verify that the endpoint $endpointPartialUpdateWithId returns 200")
        steps.add("Verify that the endpoint $endpointPartialUpdateWithId contains $output")
    ...
    private fun createTestCaseBadRequest400ID(invalidIDDataTest: DataTest?, script: Script, entity: Entity): TestCase {
        ...
        steps.add("The input request for the PATCH endpoint must be strictly: $requestParameter")
        steps.add("Verify that the endpoint $endpointPartialUpdateWithId returns 400")
        steps.add("Verify that the endpoint $endpointPartialUpdateWithId contains the message: $errorMessage")
        negativeTestCase.steps.addAll(steps)
        return negativeTestCase
    }
    ...
    private fun createTestCaseInternalError500(ruleDataTest: ListDataTest, script: Script, entity: Entity): TestCase? {
        ...
        val idAttribute = entity.attributes.find { it.name.startsWith("id") }?.name as String
        val endpointPartialUpdate = testArchitecture.endpoints["partial update"]?.replace("{id}", idAttribute)
        val endpointPartialUpdateWithId = testArchitecture.endpoints["partial update"]?.replace("{id}", validDataTest?.input?.get(idAttribute).toString())


        if(validDataTest != null) {
            val expectedResult = "${script.location} mapped by PATCH $endpointPartialUpdate returns internal error 500"
            ...
            val errorMessage = "Failed to update ${entity.name} in the database"
            ...
            steps.add("Call method ${testArchitecture.controllerLayerMethod} mapped by PATCH $endpointPartialUpdateWithId in ${testArchitecture.controllerLayerClass} class and return internal error code 500 with the following error message: $errorMessage")
            steps.add("The input request for the PATCH endpoint must be strictly: $requestParameter")

            steps.add("Verify that ${testArchitecture.ruleLayerMethod} was called")
            steps.add("Verify that the endpoint $endpointPartialUpdateWithId returns 500")
            steps.add("Verify that the endpoint $endpointPartialUpdateWithId contains the message: $errorMessage")
    ...
    private fun createTestCaseInstanceNotFound404(ruleDataTest: ListDataTest, script: Script, entity: Entity): TestCase? {
        ...
        if(validDataTest != null) {
            val expectedResult = "${script.location} mapped by PATCH $endpointPartialUpdate does not partial update an instance of ${entity.name} then returns not found 404"
            ...
            steps.add("Call method ${testArchitecture.controllerLayerMethod} mapped by PATCH $endpointPartialUpdateWithId in ${testArchitecture.controllerLayerClass} class and It returns not found code 404 with the following message: $errorMessage")
            steps.add("The input request for the PATCH endpoint must be strictly: $requestParameter")
            steps.add("Verify that ${testArchitecture.ruleLayerMethod} was called")
            steps.add("Verify that the endpoint $endpointPartialUpdateWithId returns 404")
            steps.add("Verify that the endpoint $endpointPartialUpdateWithId contains the message: $errorMessage")
